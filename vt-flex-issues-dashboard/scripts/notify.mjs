...
function getConfig() {
  return {
    ...
    dashboardBaseUrl:    (process.env.DASHBOARD_BASE_URL || '').replace(/\/$/, '')
  };
}
...
function buildPlaintext({ issue, triageMd, cfg }) {
  const dashboardLine = cfg.dashboardBaseUrl
    ? `\nView full report: ${cfg.dashboardBaseUrl}/issues/${issue.issue_id}\n`
    : '';
  return [
    `Rep:        ${issue.agent_name || 'unknown'} (${issue.worker_email || 'no email'})`,
    `Team:       ${issue.team_name || 'unknown'}`,
...
function buildHtml({ issue, triageMd, cfg }) {
  const triageHtml  = markdownToHtml(triageMd);
  const desc        = escapeHtml(issue.agent_description || '(none provided)');
  const dashboardLn = cfg.dashboardBaseUrl
    ? `<p style="margin:8px 0 0;font-size:12px;color:#666;"><a href="${escapeHtml(cfg.dashboardBaseUrl)}/issues/${escapeHtml(issue.issue_id)}">View full report in dashboard</a></p>`
    : '';
